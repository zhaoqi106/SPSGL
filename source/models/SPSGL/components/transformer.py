...
class TransformerEncoderLayer(Module):
    ...
    def forward(
            ...
            src_key_padding_mask: Optional[Tensor] = None,
            is_causal: bool = False, weight_matrix = None) -> Tensor:
        ...
        x = src
        if self.norm_first:
            x = x + self._sa_block(self.norm1(x), src_mask, src_key_padding_mask, weight_matrix = weight_matrix)
            x = x + self._ff_block(self.norm2(x))
        else:
            x = self.norm1(x + self._sa_block(x, src_mask, src_key_padding_mask, weight_matrix = weight_matrix))
            x = self.norm2(x + self._ff_block(x))
        return x

    # self-attention block
    def _sa_block(self, x: Tensor,
                  attn_mask: Optional[Tensor], key_padding_mask: Optional[Tensor],weight_matrix: Optional[Tensor] ) -> Tensor:
