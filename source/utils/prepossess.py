...
def add_gaussian_noise(fc_tensor, std=0.01, prob=0.5):
    """对FC矩阵添加高斯噪声（带随机概率）"""
    if np.random.rand() < prob:
        noise = torch.randn_like(fc_tensor) * std
        return fc_tensor + noise
    return fc_tensor

def drop_edge(fc_tensor, drop_prob=0.05, apply_prob=0.5):
    """随机丢边（设置部分连接为0，带随机概率）"""
    if np.random.rand() < apply_prob:
        mask = (torch.rand_like(fc_tensor) > drop_prob).float()
        mask = torch.triu(mask, diagonal=1)
        mask = mask + mask.transpose(-1, -2)
        return fc_tensor * mask
    return fc_tensor
